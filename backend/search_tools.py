...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        formatted = []
        sources_with_links = []  # Track sources with links
        ...
        for doc, meta in zip(results.documents, results.metadata, strict=True):
            ...
            # Try to get lesson link if lesson_number exists
            source_url = None
            if lesson_num is not None:
                try:
                    source_url = self.store.get_lesson_link(course_title, lesson_num)
                except Exception:
                    # If link retrieval fails, source will have no URL
                    pass

            # Store source with optional link
            sources_with_links.append({"title": source_title, "url": source_url})
